...
const form = useForm({
    ...
    observation: props.period.observation,
    is_active: Boolean(props.period.is_active)
});

const submit = () => {
    form.put(route('periods.update', props.period), {
        preserveScroll: true,
        onSuccess: () => toast.success('Período actualizado com sucesso.'),
        onError: () => toast.error('Ocorreu um erro ao tentar actualizar período.')
    });
};
